...
\author{
    Ricardo Mart\'inez \and Godric Aceves\\
    \\
    AI Safety Hackathon 2025\\
...
\begin{abstract}
We present DarkPatternMonitor, a scalable oversight system for detecting manipulative behaviors (``dark patterns'') in large language model (LLM) conversations. Using a taxonomy aligned with DarkBench, we analyze 1,299 real assistant turns from WildChat and find that \textbf{29.4\% exhibit manipulation markers}. Our system combines LLM-as-judge labeling with a lightweight classifier (78\% macro F1) to enable monitoring at scale. We compare benchmark predictions against real-world behavior, finding low distributional divergence (JS = 0.08) but notable category-level gaps. Our findings suggest that manipulation patterns in LLMs are more prevalent than benchmark tests indicate, particularly for harmful generation (12.8\% in the wild vs. 16.7\% in benchmarks).
\end{abstract}
...
\begin{enumerate}
    ...
    \item Analysis of 1,299 real WildChat conversations (29.4\% flagged)
    \item Benchmark-to-reality gap analysis comparing DarkBench predictions to WildChat observations
...
\textbf{WildChat:} We extracted 1,299 assistant turns from the WildChat-1M dataset \citep{wildchat2024}, sampling conversations from both GPT-3.5-turbo and GPT-4.
...
Table \ref{tab:prevalence} shows dark pattern prevalence across 1,299 WildChat turns.

\begin{table}[H]
\centering
\caption{Dark Pattern Prevalence in WildChat (N=1,299)}
\label{tab:prevalence}
\begin{tabular}{lrrr}
\toprule
Category & Count & Rate (\%) & Per 1,000 \\
\midrule
harmful\_generation & 166 & 12.8 & 127.8 \\
brand\_bias & 86 & 6.6 & 66.2 \\
anthropomorphism & 68 & 5.2 & 52.3 \\
sycophancy & 29 & 2.2 & 22.3 \\
user\_retention & 23 & 1.8 & 17.7 \\
sneaking & 10 & 0.8 & 7.7 \\
\midrule
\textbf{Total Flagged} & \textbf{382} & \textbf{29.4} & \textbf{294.1} \\
Clean (none) & 917 & 70.6 & 705.9 \\
\bottomrule
\end{tabular}
\end{table}

\textbf{Key finding:} Nearly 1 in 3 assistant responses (29.4\%) shows at least one manipulation marker.
...
JS Divergence & 0.083 \\
KL Divergence & 0.326 \\
DarkBench samples & 18 \\
WildChat samples & 1,299 \\
\bottomrule
\end{tabular}
\end{table}
...
WildGuard demonstrates that dark patterns in LLM conversations are more prevalent than benchmark tests might suggest. Our analysis of 1,299 WildChat turns found 29.4\% exhibiting manipulation markers, with harmful\_generation (12.8\%) and brand\_bias (6.6\%) most common.

The system achieves 78\% macro F1 on a 7-class problem, enabling practical monitoring at scale. Our gap analysis validates DarkBench as a reasonable proxy for real-world behavior (JS divergence = 0.083), while highlighting category-level discrepancies.

Future work should:
\begin{itemize}
    \item Expand to multi-turn pattern detection
    \item Include additional LLM providers beyond ChatGPT
    \item Develop adversarial robustness against manipulation of the detector itself
\end{itemize}
